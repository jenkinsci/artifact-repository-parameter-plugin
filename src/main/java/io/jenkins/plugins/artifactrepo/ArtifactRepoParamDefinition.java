...
public class ArtifactRepoParamDefinition extends ParameterDefinition {
    ...
    public ArtifactRepoParamDefinition(
        ...
        this.credentialsId =
                Optional.ofNullable(credentialsId).map(String::trim).orElse("");
        ...
        this.resultsCount = Optional.ofNullable(resultsCount)
                ...
                .map(Integer::valueOf)
                .orElse(10);
        this.filterRegex = Optional.ofNullable(filterRegex)
    ...
    private Map<String, ResultEntry> markPreselectedEntries(Map<String, ResultEntry> entries) {
        ...
        switch (selectEntry) {
            case "first":
                entries.entrySet().stream().findFirst().ifPresent(e -> e.getValue()
                        .setSelected(true));
                break;
            case "last":
                entries.entrySet().stream().reduce((first, second) -> second).ifPresent(e -> e.getValue()
                        .setSelected(true));
                break;
            case "regex":
    ...
    private String getValuesFromJson(JSONObject json) {
        ...
        if (jsonValue instanceof String) {
            ...
            return ((JSONArray) jsonValue).stream().map(Object::toString).collect(Collectors.joining("\n"));
        }
